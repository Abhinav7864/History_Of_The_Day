...
export default function HeroSection({ todayEvent, yesterdayEvent }) {
	...
	const handleTodayClick = () => {
		...
		setTimeout(() => {
			document.getElementById('todays-chronicle')?.scrollIntoView({ behavior: 'smooth' });
		}, 400);
		...
		<section className="w-full max-w-7xl mx-auto px-6 pt-12 pb-4 md:pt-16 md:pb-4 lg:pt-20 lg:pb-4 grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24 items-center">
			...
			<div className="flex flex-col items-start text-left space-y-8">
				<div className="flex flex-col items-center w-full md:w-auto">
					...
					<h1 className="font-display text-5xl md:text-7xl text-ink leading-none text-center">
						History<br />
						<span className="text-3xl md:text-5xl text-ink/30 block mt-2">of the Day</span>
					</h1>
				</div>
			...
			<div className="relative w-full h-full min-h-[400px] flex items-center justify-center rounded-2xl p-8 md:p-12 overflow-hidden">
				...
				<div
					...
					style={{
						background: 'radial-gradient(ellipse at center, transparent 70%, rgba(139, 115, 85, 0.08) 100%)'
					}}
				></div>
				...
				<div className="relative z-10 flex gap-6 items-center justify-center w-full">
					...
					<div className="w-[260px] h-[347px] flex-shrink-0 relative">
						...
						<div
							...
							className={`absolute inset-0 rounded-sm p-6 flex flex-col justify-between transition-all duration-300 cursor-pointer origin-center ${isYesterdayHovered
						...
						>
							<span
								className={`absolute top-0 left-0 w-full h-1 bg-gold transition-opacity duration-300 ${isYesterdayHovered ? 'opacity-100' : 'opacity-0'
									}`}
							></span>
							<div>
								<span className={`font-display text-xs uppercase tracking-widest block mb-2 transition-colors duration-300 ${isYesterdayHovered ? 'text-ink/40' : 'text-ink/30'
									}`}>Yesterday</span>
								<h3 className={`font-display leading-tight mb-2 line-clamp-3 transition-all duration-300 ${isYesterdayHovered ? 'text-2xl text-ink' : 'text-xl text-ink/60'
									}`}>
									{yesterdayEvent ? yesterdayEvent.title : "No Event"}
								</h3>
							</div>
							<div className={`mt-auto pt-4 border-t transition-colors duration-300 flex justify-between items-baseline ${isYesterdayHovered ? 'border-ink/10' : 'border-ink/5'
								}`}>
								<span className={`font-display transition-colors duration-300 ${isYesterdayHovered ? 'text-4xl text-ink/20' : 'text-4xl text-ink/10'
									}`}>{yesterdayEvent?.day}</span>
								<span className={`font-body text-sm transition-colors duration-300 ${isYesterdayHovered ? 'text-ink/50' : 'text-ink/40'
									}`}>{yesterdayEvent?.year}</span>
							</div>
						</div>
					</div>
					...
					<div className="w-[260px] h-[347px] flex-shrink-0 relative">
						{/* Today Card */}
						<div
							className={`absolute inset-0 rounded-sm p-6 flex flex-col justify-between transition-all duration-300 origin-center ${isYesterdayHovered
								? 'bg-paper/60 border border-ink/5 shadow-sm grayscale opacity-70 scale-95'
						...
						>
							<span
								className={`absolute top-0 left-0 w-full h-1 bg-gold transition-opacity duration-300 ${isYesterdayHovered ? 'opacity-0' : 'opacity-100'
									}`}
							></span>
							<div>
								<span className={`font-display text-xs uppercase tracking-widest block mb-2 transition-colors duration-300 ${isYesterdayHovered ? 'text-ink/30' : 'text-ink/40'
									}`}>Today</span>
								<h3 className={`font-display leading-tight mb-2 line-clamp-3 transition-all duration-300 ${isYesterdayHovered ? 'text-xl text-ink/60' : 'text-2xl text-ink'
									}`}>
									{todayEvent ? todayEvent.title : "Loading..."}
								</h3>
							</div>
							<div className={`mt-auto pt-4 border-t transition-colors duration-300 flex justify-between items-baseline ${isYesterdayHovered ? 'border-ink/5' : 'border-ink/10'
								}`}>
								<span className={`font-display transition-colors duration-300 ${isYesterdayHovered ? 'text-4xl text-ink/10' : 'text-4xl text-ink/20'
									}`}>{todayEvent?.day}</span>
								<span className={`font-body text-sm transition-colors duration-300 ${isYesterdayHovered ? 'text-ink/40' : 'text-ink/50'
									}`}>{todayEvent?.year}</span>
							</div>
						</div>
					</div>
