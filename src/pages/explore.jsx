...
export default function ExplorePage() {
	...
	useEffect(() => {
		setLoading(true);
		historyService.getEventsByDate(selectedDate.month, selectedDate.day)
			.then(data => {
				setEvents(data);
				setLoading(false);
			});
			...
			<div className="grid lg:grid-cols-12 gap-12">
				...
				<div className="lg:col-span-8">
					<h2 className="text-2xl text-ink font-display uppercase mb-8 border-l-4 border-gold pl-4 tracking-widest flex items-center gap-4">
						Events <span className="text-gold-dipped text-lg">// {selectedDate.day} . {selectedDate.month}</span>
					</h2>

					{loading ? (
						<div className="text-center py-20 border-2 border-dashed border-ink opacity-50">
							<span className="font-display text-2xl animate-pulse">Consulting the Archives...</span>
						</div>
					) : events.length > 0 ? (
						...
						<div className="text-center py-24 bg-paper-light border-rough">
							<p className="text-2xl font-display text-ink-muted opacity-60">No records found in the archive.</p>
						</div>
					)}
				</div>
